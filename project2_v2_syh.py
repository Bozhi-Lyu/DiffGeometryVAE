...
if __name__ == "__main__":
    ...
    if args.mode == 'train':
        ...
        # # Test load models
        # testlist = [ VAE(prior, decoder, encoder) ] * 10
        # testlist = torch.load('ensemblemodels.pt')
        # print(testlist)
        ...
        for k in range(num_curves):
            ...
            def closure():
                ...
                def kl_divergence_between_decoders(decoder, z1, z2):
                    # Get the distributions from each decoder
                    dist1 = decoder(z1)
                    dist2 = decoder(z2)

                    # Compute the KL divergence between the two distributions
                    kl_div = td.kl_divergence(dist1, dist2)
                ...
                for i in range(num_points-1):
                    ...
                    kl_div = kl_divergence_between_decoders(decoder, curve[i], curve[i + 1])

                    # Add the KL divergence to the total energy
                    energy += kl_div
                ...
                # #########################
                # delta_s = curve[1:] - curve[:-1]
                #
                # I = torch.eye(2, 2)[None, :, :].repeat(num_points-1, 1, 1)
                # cat = torch.cat((z_norm, torch.norm(parameters, dim=-1)))
                # G = (1 + cat ** 2)[:, None, None] * I
                #
                # squared_curve_velocity = torch.bmm(G, delta_s.unsqueeze(-1)).squeeze(-1)#delta_s.T @ G @ delta_s
                # energy = torch.sum(squared_curve_velocity * delta_s)
                # energy.backward()
                # ##########################
